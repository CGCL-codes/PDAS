...
deafult_cfg0 = [16, 16, 16, 16, 32, 32, 32, 64, 64, 64]
default_cfg1 = [16, 16, 16, 16, 32, 32, 32, 64, 64, 64]
default_cfg2 = [16, 32, 64]
...
class Architect(object):
    ...
    def __init__(self, model, criterion, total_params, conv_list, other_list, args):
    ...
    def _compute_unrolled_model(self, inputs, targets, eta, network_optimizer):
        ...
        ratio1 = [default_cfg1[j] / default_cfg0[j] for j in range(len(default_cfg1))]
        ratio2 = [(prune_ratio[j][index[j][0].item()] + 1) * 2 / default_cfg2[j] for j in range(len(prune_index))]
        ratio2 = [val for val in ratio2 for i in range(3)]
        ratio = []
        ratio.append(ratio1[0])
        for j in range(len(ratio2)):
            ratio.append(ratio1[j+1])
            ratio.append(ratio2[j])
        count_params = 0
        for j in range(len(ratio)):
            if j == 0:
                count_params += self.conv_list[j] * ratio[j]
            else:
                count_params += self.conv_list[j] * ratio[j-1] * ratio[j]
        count_params += sum(self.other_list)
        if count_params > (0.53 * self.total_params):
            #param_loss = count_params / (0.6 * self.total_params)
            param_loss = 3 * math.log(count_params / (0.5 * self.total_params))
        elif count_params < (0.47 * self.total_params):
            #param_loss = count_params / (0.6 * self.total_params)
            param_loss = 3 * math.log(count_params / (0.5 * self.total_params))
        else:
            param_loss = 0
    ...
    def _backward_step_unrolled(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer):
        ...
        unrolled_loss = basic_loss + param_loss
