...
class Architect(object):
    ...
    def __init__(self, model, criterion, count_params, args):
    ...
    def _compute_unrolled_model(self, inputs, targets, eta, network_optimizer):
        ...
        total_params = self.count_model_params(self.model)
            if total_params > (0.63 * self.count_params):
                param_loss = total_params / (0.6 * self.count_params)
            elif total_params < (0.57 * count_params):
                param_loss = (0.6 * self.count_params) / total_params
            else:
                param_loss = 1
    ...
    def _backward_step_unrolled(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer):
        unrolled_model = self._compute_unrolled_model(input_train, target_train, eta, network_optimizer, criterion)
        logit_valid = unrolled_model(input_valid)
        basic_loss = self.criterion(logit_valid, target_valid)
        total_params = self.count_model_params(unrolled_model)
            if total_params > (0.63 * self.count_params):
                param_loss = total_params / (0.6 * self.count_params)
            elif total_params < (0.57 * count_params):
                param_loss = (0.6 * self.count_params) / total_params
