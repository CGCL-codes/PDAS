...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, weights):
        ...
        '''x0 = channel_shuffle(x, x.shape[1] // 2)
        out_channels = self.cfg
        out = self.conv1(x)
        out = torch.zeros_like(out)
        weight_copy = self.conv1.weight.data.clone()
        weight_vec = weight_copy.view(weight_copy.shape[0], -1)
        norm2 = torch.norm(weight_vec, 2, 1)'''
        ...
        '''for j in range(3):
            remainde_num = choose_channels[prune_ratio[self.index_p][j]]
            _, idx = norm2.topk(remained_num)
            remained_weights = self.conv1.weight.data[idx.tolist(), :, :, :].clone()
            pruned_weights = torch.zeros(self.conv1.weight.data.shape[0]-remained_num, self.conv1.weight.data.shape[1], self.conv1.weight.data.shape[2], self.conv1.weight.data.shape[3]).cuda(torch.cuda.current_device())
            complete_weights = torch.cat((remained_weights, pruned_weights), dim=0)
            self.conv1.weight.data = complete_weights.clone()
            conv_out = self.conv1(x0)[:, :remained_num]
            out += ChannelWiseInterV2(conv_out, out_channels) * weights[self.index_p][j]
            self.conv1.weight.data = weight_copy.clone()'''

        out_channels = self.cfg
        out = self.conv1(x)
        out = channel_shuffle(out, out.size(1) // 2)
        choices = [choose_channels[prune_ratio[self.index_p][j]] for j in range(3)]
        ...
        out = outA * weights[self.index_p][0] + outB * weights[self.index_p][1] + outC * weights[self.index_p][2]

        out = self.bn1(out)
        out = self.relu(out)

        '''out0 = channel_shuffle(out, out.shape[1] // 2)
        out_channels = self.planes
        out = self.conv2(out)
        out = torch.zeros_like(out)
        weight_copy = self.conv2.weight.data.clone()
        weight_vec = weight_copy.view(weight_copy.shape[0], -1)
        norm2 = torch.norm(weight_vec, 2, 1)

        for j in range(3):
            remainde_num = choose_channels[prune_ratio[self.index_p+1][j]]
            _, idx = norm2.topk(remained_num)
            remained_weights = self.conv2.weight.data[idx.tolist(), :, :, :].clone()
            pruned_weights = torch.zeros(self.conv2.weight.data.shape[0]-remained_num, self.conv2.weight.data.shape[1], self.conv2.weight.data.shape[2], self.conv2.weight.data.shape[3]).cuda(torch.cuda.current_device())
            complete_weights = torch.cat((remained_weights, pruned_weights), dim=0)
            self.conv2.weight.data = complete_weights.clone()
            conv_out = self.conv2(out0)[:, :remained_num]
            out += ChannelWiseInterV2(conv_out, out_channels) * weights[self.index_p+1][j]
            self.conv2.weight.data = weight_copy.clone()'''
        
        out_channels = self.planes
        out = self.conv2(out)
        out = channel_shuffle(out, out.size(1) // 2)
        choices = [choose_channels[prune_ratio[self.index_p+1][j]] for j in range(3)]
        #choices = [max(int(round((out_channels * (1 - prune_ratio[self.index_p][j])) / 2) * 2), 2) for j in range(5)]
        out_convs = [out[:, :oC] for oC in choices]
        outA = ChannelWiseInterV2(out_convs[0], out_channels)
        outB = ChannelWiseInterV2(out_convs[1], out_channels)
        ...
        #outD = ChannelWiseInterV2(out_convs[3], out_channels)
        #outE = ChannelWiseInterV2(out_convs[4], out_channels)
        out = outA * weights[self.index_p+1][0] + outB * weights[self.index_p+1][1] + outC * weights[self.index_p+1][2]
...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, dataset='cifar10', cfg=None):
        ...
        self.layer2 = self._make_layer(block, 32, n, 2, cfg=cfg[n+1:2*n+1], stride=2)
        self.layer3 = self._make_layer(block, 64, n, 4, cfg=cfg[2*n+1:3*n+1], stride=2)
    ...
    def _make_layer(self, block, planes, blocks, index, cfg, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, self.n*index+1, cfg[0], stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, self.n*index+2*i+1, cfg[i]))
    ...
    def forward(self, x):
        ...
        for j in range(5):
            remained_num = choose_channels[prune_ratio[0][j]]
            _, idx = norm2.topk(remained_num)
            remained_weights = self.conv1.weight.data[idx.tolist(), :, :, :].clone()
            pruned_weights = torch.zeros(self.conv1.weight.data.shape[0]-remained_num, self.conv1.weight.data.shape[1], self.conv1.weight.data.shape[2], self.conv1.weight.data.shape[3]).cuda(torch.cuda.current_device())
            complete_weights = torch.cat((remained_weights, pruned_weights), dim=0)
            self.conv1.weight.data = complete_weights.clone()
            conv_out = self.conv1(x)[:, :remained_num]
            out += ChannelWiseInterV2(conv_out, out_channels) * weights[0][j]
            self.conv1.weight.data = weight_copy.clone()'''
        ...
        out = self.conv1(x)
        out = channel_shuffle(out, out.size(1) // 2)
        choices = [choose_channels[prune_ratio[0][j]] for j in range(3)]
        #choices = [max(int(round((out_channels * (1 - prune_ratio[0][j])) / 2) * 2), 2) for j in range(5)]
        out_convs = [out[:, :oC] for oC in choices]
        outA = ChannelWiseInterV2(out_convs[0], out_channels)
        outB = ChannelWiseInterV2(out_convs[1], out_channels)
        outC = ChannelWiseInterV2(out_convs[2], out_channels)
        #outD = ChannelWiseInterV2(out_convs[3], out_channels)
        #outE = ChannelWiseInterV2(out_convs[4], out_channels)
        out = outA * weights[0][0] + outB * weights[0][1] + outC * weights[0][2]
    ...
    def _initialize_alphas(self):
        k = len(prune_index)
        self.arch_params = Variable(1e-3 * torch.randn(k, 3).cuda(), requires_grad=True)
