...
from architect164_1 import Architect


parser = argparse.ArgumentParser(description='Cifar10 Train Search')
parser.add_argument('--data', type=str, default='/home/user/workspace/cifar100',
...
base_model = resnet(depth=164, dataset='cifar100')
...
def main(round_num):
    ...
    count_ops, count_params, conv_list, other_list = measure_model(depth=164, dataset='cifar100')
    print('=> count_ops: {}, count_params: {}, conv list: {}, other list: {}'.format(count_ops, count_params, conv_list, other_list))
    #print(len(conv_list))
    #exit()

    model = resnet(depth=164, dataset='cifar100')
    ...
    for epoch in range(args.epochs):
        ...
        train_acc, train_loss, train_param_loss = train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, epoch, round_num, count_params, count_ops, conv_list)
        scheduler.step()
                ...
                for j in range(len(prune_index)):
                    ...
                    if j < 19:
                        while(a in prune_ratio[j] and num <= 3):
                            a = random.randint(max(max_ratio-2, 1), min(max_ratio+2, len(channel16)-1))
                            num += 1
                    elif j < 37:
                        while(a in prune_ratio[j] and num <= 3):
                            a = random.randint(max(max_ratio-5, 2), min(max_ratio+5, len(channel32)-1))
                            num += 1
                    else:
                        while(a in prune_ratio[j] and num <= 3):
                            a = random.randint(max(max_ratio-10, 3), min(max_ratio+10, len(channel64)-1))
                            num += 1
                    if min_ratio[j][0] >= 3:
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, epoch, round_num, count_params, count_ops, conv_list):
    losses = AverageMeter()
