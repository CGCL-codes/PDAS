...
from architect4 import Architect
...
parser.add_argument('--rounds', type=int, default=12, help='times to run process')
args = parser.parse_args()

log = open(os.path.join(args.save, 'channel-search-resnet32-0915.txt'),'w')
...
cfg = [6, 16, 2, 6, 16, 16, 32, 32, 12, 22, 22, 24, 64, 64, 44, 2]

base_model = resnet(depth=32, cfg=cfg)
...
def main(round_num):
    ...
    count_ops, count_params, conv_list, other_list = measure_model(depth=32, cfg=[6, 16, 16, 2, 16, 6, 16, 16, 16, 16, 16, 32, 32, 32, 32, 12, 32, 22, 32, 22, 32, 24, 64, 64, 64, 64, 64, 44, 64, 2, 64])
    print('=> count_ops: {}, count_params: {}, conv list: {}, other list: {}'.format(count_ops, count_params, conv_list, other_list))

    model = resnet(depth=32, cfg=cfg)
                ...
                for j in range(len(prune_index)):
                    ...
                    if j < 1:
                        while(a in prune_ratio[j] and num <= 3):
                            a = random.randint(max(max_ratio-2, 2), min(max_ratio+2, len(channel16)-1))
                            num += 1
                    elif j < 2:
                        while(a in prune_ratio[j] and num <= 3):
