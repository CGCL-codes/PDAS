...
parser.add_argument('--data', type=str, default='/home/user/workspace/cifar10',
...
log = open(os.path.join(args.save, 'channel-search-resnet56-0915.txt'),'w')
...
#cfg = [16, 2, 16, 10, 22, 22, 12, 64, 44, 2]
#cfg = [8, 8, 12, 16, 4, 4, 24, 24, 8, 32, 16, 48, 48, 64, 32, 16]
#cfg = [8, 4, 10, 16, 8, 10, 4, 12, 4, 4, 32, 32, 22, 28, 14, 32, 24, 22, 20, 64, 8, 54, 50, 26, 16, 44, 8, 26]
cfg = [14, 4, 8, 4, 6, 14, 8, 12, 14, 12, 10, 12, 8, 16, 6, 14, 10, 4, 6, 30, 30, 6, 26, 24, 6, 14, 32, 14, 16, 6, 16, 6, 30, 26, 14, 26, 12, 44, 24, 64, 64, 56, 28, 56, 32, 26, 18, 8, 44, 8, 8, 12, 8, 24, 8]

default_cfg = []
default_cfg.append(cfg[0])
for j in range(3):
    for k in range(1, 19):
        default_cfg.append(cfg[18*j+k])
        if j == 0:
...
base_model = resnet(depth=110, cfg=cfg)
...
def main(round_num):
    ...
    count_ops, count_params, conv_list, other_list = measure_model(depth=110, cfg=default_cfg)
    print('=> count_ops: {}, count_params: {}, conv list: {}, other list: {}'.format(count_ops, count_params, conv_list, other_list))

    model = resnet(depth=110, cfg=cfg)
                ...
                for j in range(len(prune_index)):
                    ...
                    if j < 1:
                        while(a in prune_ratio[j] and num <= 3):
                            a = random.randint(max(max_ratio-1, 6), min(max_ratio+1, len(channel16)-1))
                            num += 1
                    elif j < 2:
                        while(a in prune_ratio[j] and num <= 3):
                        ...
                        while(a in prune_ratio[j] and num <= 3):
                            a = random.randint(max(max_ratio-6, 15), min(max_ratio+6, len(channel64)-1))
                            num += 1
                    if min_ratio[j][0] >= 3:
