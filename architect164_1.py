...
default_cfg = [16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 16, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 32, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64, 64]
...
class Architect(object):
    ...
    def _backward_step(self, input_valid, target_valid):
        ...
        for j in range(len(ratio)):
            if j == 0:
                ...
                count_ops += self.conv_list[3*j-2] * ratio[j-1] * ratio[j] + self.conv_list[3*j-1] * ratio[j] * ratio[j] + self.conv_list[3*j] * ratio[j]
            else:
                count_ops += self.conv_list[3*j-2] * ratio[j] + self.conv_list[3*j-1] * ratio[j] * ratio[j] + self.conv_list[3*j] * ratio[j]

        count_ops += sum(self.other_list)
        if count_ops > (0.65 * self.total_ops):
            param_loss = 4 * math.log(count_ops / (0.625 * self.total_ops))
        elif count_ops < (0.6 * self.total_ops):
            param_loss = -4 * math.log(count_ops / (0.625 * self.total_ops))
        else:
            param_loss = 0
    ...
    def _backward_step_unrolled(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer):
        ...
        ratio = [(prune_ratio[j][index[j][0].item()] + 1) * 2 / default_cfg[j] for j in range(len(prune_index))]
        count_params = 0
        for j in range(len(self.conv_list)):
            if j == 0:
                count_params += self.conv_list[j] * ratio[j]
            elif j == 3:
                count_params += self.conv_list[1] * ratio[0] * ratio[1] + self.conv_list[2] * ratio[1] * ratio[2] + self.conv_list[3] * ratio[2]
            elif j % 3 == 0:
                count_params += self.conv_list[j-2] * ratio[(j//3)*2-1] + self.conv_list[j-1] * ratio[(j//3)*2-1] * ratio[(j//3)*2] + self.conv_list[j] * ratio[(j//3)*2]
