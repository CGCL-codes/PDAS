...
class Architect(object):
    ...
    def _compute_unrolled_model(self, inputs, targets, eta, network_optimizer):
        ...
        ratio = [(prune_ratio[j][index[j][0].item()] + 1) * 2 / cfg[j] for j in range(len(prune_index))]
        count_params = 0
        count_params += (self.conv_list[0] + self.conv_list[1])
        count0 = sum(self.conv_list[j] for j in range(2, 12))
        count1 = sum(self.conv_list[j] for j in range(12, 22))
        count2 = sum(self.conv_list[j] for j in range(22, len(self.conv_list)))
        count_params += (count0 * ratio[0] + count1 * ratio[1] + count2 * ratio[2])
        count_params += sum(self.other_list)
        if count_params > (0.89 * self.total_params):
            #param_loss = count_params / (0.6 * self.total_params)
            param_loss = 3 * math.log(count_params / (0.86 * self.total_params))
        elif count_params < (0.83 * self.total_params):
            #param_loss = count_params / (0.6 * self.total_params)
            param_loss = 3 * math.log(count_params / (0.86 * self.total_params))
        else:
            param_loss = 0
    ...
    def _backward_step(self, input_valid, target_valid):
        ...
        arch_weights = F.softmax(self.model.module.arch_params, dim=-1)
        _, index = arch_weights.topk(1, 1, True, True)
        ratio = [(prune_ratio[j][index[j][0].item()] + 1) * 2 * 4 / cfg[j] for j in range(len(prune_index))]
        count_ops = 0
        count_ops += (self.conv_list[0] + self.conv_list[1] + self.conv_list[2] + self.conv_list[3] * ratio[0]) 
        count_ops += sum(self.conv_list[3*j+1] * ratio[0] + self.conv_list[3*j+2] + self.conv_list[3*j+3] * ratio[0] for j in range(1, 18))
        count_ops += (self.conv_list[55] * ratio[0] + self.conv_list[56] + self.conv_list[57] * ratio[1])
        count_ops += sum(self.conv_list[3*j+1] * ratio[1] + self.conv_list[3*j+2] + self.conv_list[3*j+3] * ratio[1] for j in range(19, 36))
        count_ops += (self.conv_list[109] * ratio[1] + self.conv_list[110] + self.conv_list[111] * ratio[2])
        count_ops += sum(self.conv_list[3*j+1] * ratio[2] + self.conv_list[3*j+2] + self.conv_list[3*j+3] * ratio[2] for j in range(37, 54))

        count_ops += sum(self.other_list)
        if count_ops > (0.82 * self.total_ops):
            param_loss = 4 * math.log(count_ops / (0.79* self.total_ops))
        elif count_ops < (0.76 * self.total_ops):
            param_loss = -4 * math.log(count_ops / (0.79* self.total_ops))
        else:
            param_loss = 0

        loss = basic_loss + 0.5*param_loss
        loss.backward()
    ...
    def _backward_step_unrolled(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer):
        ...
        count_params = 0
        count_params += (self.conv_list[0] + self.conv_list[1])
        count0 = sum(self.conv_list[j] for j in range(2, 8))
        count1 = sum(self.conv_list[j] for j in range(8, 14))
        count2 = sum(self.conv_list[j] for j in range(14, len(self.conv_list)))
        count_params += (count0 * ratio[0] + count1 * ratio[1] + count2 * ratio[2])
        count_params += sum(self.other_list)
        if count_params > (0.73 * self.total_params):
            #param_loss = count_params / (0.6 * self.total_params)
            param_loss = 3 * math.log(count_params / (0.7 * self.total_params))
        elif count_params < (0.67 * self.total_params):
            #param_loss = count_params / (0.6 * self.total_params)
            param_loss = 3 * math.log(count_params / (0.7 * self.total_params))
        else:
            param_loss = 0
