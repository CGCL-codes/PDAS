...
from prune_params2 import ResNet110_Channel_Prune
from utils import AverageMeter

prune_index = ResNet110_Channel_Prune.index
prune_ratio = ResNet110_Channel_Prune.prune_ratio
...
class Architect(object):
    ...
    def _backward_step(self, input_valid, target_valid):
        ...
        ratio = [(prune_ratio[j][index[j][0].item()] + 1) * 2 / cfg[j] for j in range(len(prune_index))]
        count_ops = 0
        count_ops += (self.conv_list[0] + self.conv_list[1])
        count0 = sum(self.conv_list[j] for j in range(2, 38))
        count1 = sum(self.conv_list[j] for j in range(38, 74))
        count2 = sum(self.conv_list[j] for j in range(74, len(self.conv_list)))
        count_ops += (count0 * ratio[0] + count1 * ratio[1] + count2 * ratio[2])
        count_ops += sum(self.other_list)
        if count_ops > (0.765 * self.total_ops):
            param_loss = 4 * math.log(count_ops / (0.74* self.total_ops))
        elif count_ops < (0.715 * self.total_ops):
            param_loss = -4 * math.log(count_ops / (0.74* self.total_ops))
        else:
            param_loss = 0
