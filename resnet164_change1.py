...
from prune_params1 import ResNet164_Channel_Prune
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, index_p, cfg1, cfg2, stride=1, downsample=None):
        super(Bottleneck, self).__init__()
        self.conv_1 = nn.Conv2d(inplanes, cfg1, kernel_size=1, bias=False)
        self.bn_1 = nn.BatchNorm2d(cfg1)
        self.conv_2 = nn.Conv2d(cfg1, cfg2, kernel_size=3, stride=stride,
                                padding=1, bias=False)
        self.bn_2 = nn.BatchNorm2d(cfg2)
        self.conv_3 = nn.Conv2d(cfg2, planes * 4, kernel_size=1, bias=False)
    ...
    def forward(self, x, weights):
        ...
        if self.downsample or x.size(1) != out.size(1):
            residual = downsample_basic_block(x, out)
...
def downsample_basic_block(x, y):
    if x.size(2) != y.size(2):
        x = nn.AvgPool2d(2,2)(x)
    if x.size(1) < y.size(1):
        zero_pads = torch.Tensor(x.size(0), y.size(1) - x.size(1), x.size(2), x.size(3)).zero_()
        if isinstance(x.data, torch.cuda.FloatTensor):
            zero_pads = zero_pads.cuda(torch.cuda.current_device())
...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, dataset='cifar10', cfg=None):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, 16, n, 0, cfg=cfg[1:2*n+1])
        self.layer2 = self._make_layer(block, 32, n, 1, cfg=cfg[2*n+1:4*n+1], stride=2)
        self.layer3 = self._make_layer(block, 64, n, 2, cfg=cfg[4*n+1:6*n+1], stride=2)
    ...
    def _make_layer(self, block, planes, blocks, index, cfg, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, self.n*index+1, cfg[0], cfg[1], stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, self.n*index+i+1, cfg[2*i], cfg[2*i+1]))
