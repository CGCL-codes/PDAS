...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, index_p, cfg, stride=1, downsample=False):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, index_p, cfg1, cfg2, stride=1, downsample=False):
    ...
    def forward(self, x, weights):
        ...
        choices = [choose_channels[prune_ratio[self.index_p][j]] * 4 for j in range(4)]
        out_convs = [out[:, :oC] for oC in choices]
        outA = ChannelWiseInterV2(out_convs[0], out_channels)
        outB = ChannelWiseInterV2(out_convs[1], out_channels)
        outC = ChannelWiseInterV2(out_convs[2], out_channels)
...
def downsample_basic_block(x, out):
    if x.size(2) != out.size(2):
        x = nn.AvgPool2d(2,2)(x)
    zero_pads = torch.Tensor(x.size(0), out.size(1) - x.size(1), x.size(2), x.size(3)).zero_()
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, index, cfg, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, index, cfg[0], cfg[1], stride, downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, index, cfg[2*i], cfg[2*i+1]))
