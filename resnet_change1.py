...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, weights):
        ...
        '''outA = ChannelWiseInterV1(out_convs[0], x, out_channels)
        outB = ChannelWiseInterV1(out_convs[1], x, out_channels)
        outC = ChannelWiseInterV1(out_convs[2], x, out_channels)
        outD = ChannelWiseInterV1(out_convs[3], x, out_channels)'''
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, index, cfg, stride=1):
        ...
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, self.n*index+i+1, cfg[i]))
...
def ChannelWiseInterV1(inputs, x, oC):
    batch, C, H, W = inputs.size()
    if C == oC:
        outputs = inputs
        return inputs
    else:
        xtemp = nn.functional.adaptive_avg_pool3d(x, (oC-C,H,W))
        outputs = torch.cat([inputs, xtemp], dim=1)
        return outputs
